...
public class MembershipDAO {
	...
	private static final String GET_STATUS = "SELECT * FROM membership WHERE username = ? AND clubname = ?;";

	private static final String ADD_USER = "INSERT INTO membership(username, clubname, status) \n"
			+ "VALUES(?, ?, ?);";

	private static final String UPDATE_USER = "UPDATE membership set status = ? WHERE username = ? AND clubname = ?;";
	...
	public static boolean isExecutive(String username, String clubname) {
		return getStatus(username, clubname) == 1;
	}
	
	/**
	 * Returns the status of the membership in the club. 0 if the user is a member, 1 if the user is an executive, and -1 otherwise.
	 * @param username	The username of the user.
	 * @param clubname	The clubname of the club, to check that the member is in.
	 ...
	 * @param clubname	The clubname of the club to add the user to.
	 * @return	Returns the new status of the member, which ought to be 0 unless there was an error.
	 */
	...
	/**
	 * Elevates a member of a club to executive status. The member need not already be a member of the club
	 * @param username	The username of the executive to add.
	 * @param clubname	The clubname of the club to add the executive.
	 * @return	Returns the new status of the executive, which ought to be 1 unless there was an error.
	 */
	 ...
	 * @param username	The username of the user to remove from the club.
	 * @param clubname	The club to remove the user from.
	 * @return	Returns the new status of the member, which ought to be -1 unless there was an error.
	 */
	 ...
	 * @param username	The username of the user to remove as an executive.
	 * @param clubname	The club to remove the executive from.
	 * @return	Returns the new status of the member, which ought to be 0 unless there was an error.
	 */
